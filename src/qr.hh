...
DenseMatrix<T> qr_decomposition_gram_schmidt(DenseMatrix<T>& Q) {
    // small and square matrices
    if (Q.rowsize() >= Q.colsize()) {
    ...
    // matrices n > m
        ...
        // create Q matrix with the right dimensions
        DenseMatrix<T> Q_temp(Q.rowsize(), Q.rowsize());
        // copy the elements
        for (int i = 0; i < Q_temp.rowsize(); i++) {
            for (int j = 0; j < Q_temp.colsize(); j++) {
                Q_temp(i, j) = Q(i, j);
            }
        }

        // create matrix R
        DenseMatrix<T> R(Q.rowsize(), Q.colsize());

        // start orthogonalizing
        for (int k = 0; k < Q_temp.colsize(); k++) {
            // modify all later columns with column k
            for (int j = k + 1; j < Q_temp.colsize(); j++) {
                // compute factor
                T sum_nom(0.0);
                T sum_denom(0.0);
                for (int i = 0; i < Q.rowsize(); i++) {
                    sum_nom += Q_temp(i, j) * Q_temp(i, k);
                    sum_denom += Q_temp(i, k) * Q_temp(i, k);
                }

                T alpha = sum_nom / sum_denom;
                for (int i = 0; i < Q_temp.rowsize(); i++) Q_temp(i, j) -= alpha * Q_temp(i, k);
            }
        }

        // add values to R, except main diagonal
        for (int i = 1; i < R.colsize(); i++) {
            for (int j = 0; j < i && j < R.rowsize(); j++) {
                T sum_nom(0.0);
                T sum_l2nom(0.0);
                for (int k = 0; k < Q_temp.rowsize(); k++) {
                    sum_nom += Q(k, i) * Q_temp(k, j);
                    sum_l2nom += Q_temp(k, j) * Q_temp(k, j);
                }
                sum_l2nom = sqrt(sum_l2nom);
                // add element
                R(j, i) = sum_nom / sum_l2nom;
        ...
        for (int j = 0; j < Q_temp.colsize(); j++) {
            // compute norm of column j
            T sum(0.0);
            for (int i = 0; i < Q_temp.rowsize(); i++) sum += Q_temp(i, j) * Q_temp(i, j);
            sum = sqrt(sum);
            // add main diagonal to R
            if (j < R.rowsize()) R(j, j) = sum;
            // scale Q
            for (int i = 0; i < Q_temp.rowsize(); i++) Q_temp(i, j) = Q_temp(i, j) / sum;
        }
        Q = Q_temp;
        return R;
    }
