...
/** @brief classical Runge-Kutta method (order n with n stages)
  ...
  */
  ...
  public:
    ...
    RungeKutta_n (const N& model_, DenseMatrix<double> Mat, Vector<double> BV, Vector<double> CV, double sigma_=1)
      : model(model_), u(model.size()), w(model.size()), K(Mat.rowsize ())
    {
      A = Mat;
      B = BV;
...
public:
    ...
    typedef hdnum::RungeKutta_n<N,S> Solver; // Solver type

 Ordertest(const N& model_, DenseMatrix<double> Mat, Vector<double> BV, Vector<double> CV)
...
void test_order(time_type t_0, time_type T, double h_0, int L)
{
    ...
    for (int i = 0; i++; i < L)
    {
        ...
        solver.set_dt(h_0/pow(2,i));                  // set initial time step
          ...
          hdnum::Vector<time_type> times;           // store time values here
          hdnum::Vector<hdnum::Vector<number_type> > states; // store states here
        ...
        model.u(T, Earray[i]);                // exact solution from modelproblem
